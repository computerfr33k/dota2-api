...
class match_mapper_db extends match_mapper
{
    ...
    public function load($match_id = null)
    {
        ...
        $match_info = $db->query_first_pdo($query_for_match, [$this->get_match_id()]);
        $match = new match();
        if (!$match_info) {
            return $match;
        }
        $match->set_array($match_info);
        $slots = $db->fetch_array_pdo($query_for_slots, [$this->get_match_id()]);
        $slot_ids = '';
        foreach ($slots as $slot) {
            $slot_ids .= $slot['id'].',';
        }
        ...
        foreach ($additional_units as $additional_unit) {
            if (!isset($additional_units_formatted[$additional_unit['slot_id']])) {
                $additional_units_formatted[$additional_unit['slot_id']] = [];
            }
        ...
        if ($match->get('game_mode') == match::CAPTAINS_MODE) {
            ...
            $picks_bans = $db->fetch_array_pdo($query_for_picks_bans, [$match->get('match_id')]);
    ...
    public function insert(match $match)
    {
        ...
        if ($match->get('radiant_team_id')) {
            $db->insert_pdo(db::real_tablename('teams'), [
        ...
        foreach ($slots as $slot) {
            if ($slot->get('account_id') != player::ANONYMOUS) {
                $db->insert_pdo(db::real_tablename('users'), [
        ...
        if ($match->get('game_mode') == match::CAPTAINS_MODE) {
            ...
            $db->insert_many_pdo(db::real_tablename('picks_bans'), ['match_id', 'is_pick', 'hero_id', 'team', 'order'], $data);
        }
    }
    ...
    public function update($match, $lazy = true)
    {
        ...
        $db->update_pdo(db::real_tablename('matches'), $match->get_data_array(), ['match_id' => $match->get('match_id')]);
        foreach ($slots as $slot) {
            // update accounts
            $db->update_pdo(db::real_tablename('users'), [
                'account_id' => $slot->get('account_id'),
                'steamid'    => player::convert_id($slot->get('account_id')),
            ], ['account_id' => $slot->get('account_id')]);
            // update slots
            if (!$lazy) {
                $db->update_pdo(db::real_tablename('slots'), $slot->get_data_array(), ['match_id' => $slot->get('match_id'), 'player_slot' => $slot->get('player_slot')]);
            }
        }
    }
    ...
    public function delete($match_id)
    {
        ...
        $db = db::obtain();
        $slots = $db->fetch_array_pdo('SELECT id FROM '.db::real_tablename('slots').' WHERE match_id = ?', [$match_id]);
        $slots_formatted = [];
        foreach ($slots as $slot) {
        ...
        $db->delete_pdo(db::real_tablename('picks_bans'), ['match_id' => $match_id], 0);
        $db->delete_pdo(db::real_tablename('matches'), ['match_id' => $match_id]);
    }
    ...
    public static function match_exists($match_id)
    {
        $match_id = intval($match_id);
        $db = db::obtain();
        $r = $db->query_first_pdo('SELECT match_id FROM '.db::real_tablename('matches').' WHERE match_id = ?', [$match_id]);
